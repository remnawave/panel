...
Параметр `clientMtu` (при его наличии) будет преобразован в `mtu` в клиентской стороне. Таким образом, вы можете передать кастомный клиентский MTU.

Аналогичным образом, начиная с версии 2.8.0, поддерживается параметр `clientTti`. Работает он точно так же, как и `clientMtu`, но для поля `tti`.

_Параметр `clientTti` доступен только на версии 2.8.0 и выше._
...
Параметр `clientTti` (при его наличии) будет преобразован в `tti` в клиентской стороне. Таким образом, вы можете передать кастомный клиентский TTI.
