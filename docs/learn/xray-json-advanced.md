...
- **Инжектируемые хосты** (выбранные через `selector`) должны быть **включены**, но **скрыты**.
...
Объект `remnawave` добавляется на корневой уровень JSON-шаблона. Поле `injectHosts` — это **массив** групп инжекта. Каждая группа содержит **селектор** для выбора хостов и собственный `tagPrefix`:
...
| Поле        | Описание                                                |
| ----------- | ------------------------------------------------------- |
| `selector`  | Объект, определяющий какие скрытые хосты будут выбраны.  |
...
:::tip
Селекторы `remarkRegex`, `tagRegex` и `sameTagAsRecipient` работают **только** со скрытыми хостами — так же, как и `uuids`.
:::
...
- **Инжектируемые хосты должны быть включены, но скрыты.** Если хост не скрыт, выключен или не найден по селектору — он будет пропущен.
