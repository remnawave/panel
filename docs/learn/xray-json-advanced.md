...
| `addHostOutbound` | Если `true` — виртуальный хост будет добавлен как outbound с тегом `proxy` в начало массива `outbounds`. По умолчанию `false`. См. [addHostOutbound](#addhostoutbound). |
...
Если вам нужно, чтобы виртуальный хост также стал outbound'ом с тегом `proxy`, добавьте поле `addHostOutbound: true` на уровне объекта `remnawave`:

```json
"remnawave": {
    //highlight-next-line-green
    "addHostOutbound": true,
    "injectHosts": [
        {
            "selector": { "type": "uuids", "values": ["uuid-хоста-1", "uuid-хоста-2"] },
            "tagPrefix": "proxy"
...
:::tip
`addHostOutbound` можно использовать совместно с `injectHosts` или без них. Если `injectHosts` не указан или пуст, в конфиг будет добавлен только outbound виртуального хоста.
:::
...
- **Outbound'ы добавляются в начало** массива `outbounds`. Если включён `addHostOutbound`, outbound виртуального хоста с тегом `proxy` идёт первым, затем — инжектированные, затем — статические outbound'ы из шаблона (`direct`, `block`).
