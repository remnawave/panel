...
>
    ...
    <ProjectCard
        id="remnawave-xray-ui-editor"
        title="Remnawave Xray UI Editor"
        description="A visual editor for Remnawave Xray config profiles: the config is rendered as a traffic graph — squad → inbound → rule → outbound — and every node gets a form instead of hand-written JSON (VLESS/Trojan/Shadowsocks/Hysteria2, Reality with a built-in key generator, transports, routing, DNS, balancers). Before saving it validates the config with the Xray core itself, probes Reality targets over TLS, traces where a domain or IP would actually be routed, and browses geosite/geoip databases. Saves back to the panel over the API with optimistic locking and an automatic backup of every previous version."
        author="VAQYBIN"
        authorLink="https://github.com/VAQYBIN"
        image="/awesome/remnawave-xray-ui-editor.webp"
        githubRepo="VAQYBIN/Remnawave-Xray-UI-Editor"
        links={{
            github: 'https://github.com/VAQYBIN/Remnawave-Xray-UI-Editor'
        }}
    />
